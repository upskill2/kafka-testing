...
class TrackingServiceTest {
    ...
    void listen () throws ExecutionException, InterruptedException {
        ...
        trackingService.listen (buildDispatchPreparingEvent ());
        verify (kafkaTemplateMock, times (1)).send (eq (TRACKING_STATUS_TOPIC), any (TrackingStatusUpdated.class));
    }

    @Test
    void listen_throwException () throws ExecutionException, InterruptedException {
        when (kafkaTemplateMock.send (anyString (), any (TrackingStatusUpdated.class))).thenThrow (new RuntimeException ("Test Exception"));
      Exception exception = assertThrows (RuntimeException.class, () -> trackingService.listen (buildDispatchPreparingEvent ()));
       verify (kafkaTemplateMock, times (1)).send (eq (TRACKING_STATUS_TOPIC), any (TrackingStatusUpdated.class));
